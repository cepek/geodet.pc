...
C DATUM: 1989-06-30                                                   *
C * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	SUBROUTINE NGPPKA (Y,X,SIGMA,R,IER)
	DOUBLE PRECISION Y(4),X(4),SIGMA,R
	INTEGER IER	
C * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
...
C * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
C-
	DOUBLE PRECISION DX,DY,C,S,XPT,YPT,R2,Y2,XQT
C	
	IER=0
	IF (R.GT.0D0) GO TO 5
	IER=2
...
5	DX=X(2)-X(1)
	...
	XPT=DX*C+DY*S
	YPT=DY*C-DX*S
	IF(DABS(YPT).GT.R) GO TO 1
	XQT=DSQRT(R*R-YPT*YPT)
	IF (XQT.GE.15D-2*R) GO TO 2
	IER=-2
	RETURN
