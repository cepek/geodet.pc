...
C DATUM: 1989-06-30                                                   *
C * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	SUBROUTINE NGPTDB (Y,X,S,IER)
	DOUBLE PRECISION Y(4),X(4),S(2)
	INTEGER          IER
C * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
C FUNKCE: Procedura NGPTDB vypocte souradnice bodu, urcenych vodorov- *
C    nymi vzdalenostmi od dvou danych bodu. V porovnani s procedurou  *
C    NGPTDA rozeznava procedura NGPTDB vetsi pocet typu singularity   *
...
C	
	IER=0
	IF (S(1).GT.0D0.AND.S(2).GT.0D0) GO TO 4
	IER=2
	RETURN
4	DY=Y(2)-Y(1)
	DX=X(2)-X(1)
	S12=DSQRT(DX*DX+DY*DY)
	IF (S12.NE.0.) GO TO 1
	IER=1
	RETURN
1	S1=S(1)/S12
	S2=S(2)/S12
	F=(1D0+(S1+S2)*(S1-S2))/2D0
	G=(S1+F)*(S1-F)
	IF (G.GE.0.) GO TO 2
	IER=-1
	RETURN
2	H=DSQRT(G)
	IF (H.GE.15D-2*S1*S2) GO TO 3
	IER=-2
	RETURN
3	S1=DX*H
	S2=DY*H
